...
#Massive If loop - takes a Very Long Time. ------------
...
#ToDo: Make own Density plot------------
library(ggplot2)
head(ALL.dat)
probeMeans  <- c(apply(ALL.dat, 1, mean, na.rm = T), apply(CTRL.dat, 1, mean, na.rm = T))
plotterDat <- data.frame(Beta = dat.probeMeans,
           Dataset = rep(c('ALL', 'CTRL'), each = nrow(ALL.dat)))

(densityPlot <- ggplot(plotterDat, aes(x = Beta, col = Dataset)) + 
...
#Normalization and re-density plot----------
...
#ToDo: Density plot again.

dat.probeMeans <- c(apply(beta.norm[, 1:ncol(ALL.dat)], 1, mean, na.rm = TRUE),
                    apply(beta.norm[, ncol(ALL.dat):ncol(beta.norm)], 1, mean, na.rm = TRUE))
plotterNorm <-
  rbind(data.frame(plotterDat, Norm = "Before"),
        data.frame(Beta = dat.probeMeans,
                   Dataset = rep(c('ALL', 'CTRL'), each = nrow(ALL.dat)),
                   Norm = "After"))

plotterNorm$Norm <- factor(plotterNorm$Norm, levels = c("Before", "After"))
(probeAvgNorm <- ggplot(data = plotterNorm, aes(x = Beta, col = Dataset)) +
   geom_density() +
   facet_grid(Norm ~ .) +
   theme_bw()
)
