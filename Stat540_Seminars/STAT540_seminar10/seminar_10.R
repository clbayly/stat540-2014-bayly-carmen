...
#Exercise 1: perform 100 runs of this CV before selecting a model to test!------
#Add at least on model to the list of models, 
#e.g., use genes with a p-val threshold < cutoff.
#(to reveisit later)

#Exercise 2: Use AUC as a criteria to select a model based on the training data! 
#Tip: extract the predicted probabilities from each method 
#and use the roc function in ROCR.
#(to revisit later)
...
# CMA can not do a full nested cross-validation, and it is not trivial to train the selected 
#in the full dataset and then test it in the test set.
#Paul solved this problem in the following way: he made a learningsets object 
#that has just one "split" defined by the samples in the training set.
